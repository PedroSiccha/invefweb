        ...
        @foreach ($prestamo as $pr)
            ...
            <tr>
                ...
                <td class="client-status">
                    ...
                    <button type="button" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="top" title="INFORMACIÓN DEL PRESTAMO" onclick="Detalle(' {{ $pr->prestamo_id }} ', '{{ $pr->nombre }} {{ $pr->apellido }}', '{{ $pr->dni }}', '{{ $pr->garantia }}', '{{ cambiaf_a_espanol($pr->fecinicio) }}', '{{ cambiaf_a_espanol($pr->fecfin) }}', '{{ $dias }}', ' {{ $pr->monto }} ', ' {{ $pro->interesActual($dias, $pr->monto, $pr->porcentaje, $pr->morapagar)[0] }} ', ' {{ $pro->calcularMora($pr->fecfin, $pr->morapagar) }} ', ' {{ $pro->interesActual($dias, $pr->monto, $pr->porcentaje, $pr->morapagar)[2] + $pro->calcularMora($pr->fecfin, $pr->morapagar) }}', ' {{ $pro->interesActual($dias, $pr->monto, $pr->porcentaje, $pr->morapagar)[2] }} ', '{{ $pro->interesActual($dias, $pr->monto, $pr->porcentaje, $pr->morapagar)[3] }}')"><i class="fa fa-desktop"></i></button>
                </td>
            </tr>        
        @endforeach
    </tbody>
